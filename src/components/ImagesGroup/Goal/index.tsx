...
const Goal = ({image, title, content, i}:{image: StaticImageData, title: string, content: string, i:number}) =>{
    return(
        <div className={`w-full col-span-2 flex ${ i % 2 ? "justify-end" : "justify-start"}`}>
            <div className="w-full md:w-[49%] h-full border-[1px] border-[#CC0000] rounded-lg flex items-center justify-between px-4 py-3 col-span-2">
                ...
                <div className="w-3/4 h-full flex flex-col justify-start items-start">
                    ...
                    <p className="text-[65%] font-medium text-black leading-3 text-left ">{content}</p>
                </div>
            </div>
        </div>
    )
